...
リードを曲げたらダイオードの黒色の部分と、基板のシルクの横線との向きを合わせて取付けます。5個ずつくらい行っていくとよいです。 
...
ただし、右キースイッチ基板に関しては、ランドとLEDテープの端子が接触してしまう箇所があります。

そこには、LEDテープの切れ端を挟むか、LEDテープの端子部分を切り取る等して、導通しないようにしておきます。

おすすめはLEDテープの切れ端を挟むことです。
...
ルブの用意がある場合は、スタビライザーへのルブを塗布してから取り付けます。
...
画像の赤枠の部分のキースイッチは、基板から浮きやすいので注意してください。
...
ProMicro基板にはボトムプレートを仮止めしてあるので、キースイッチ基板はボトムプレートに乗った状態になります。
...
一番下等に使用する1uのキーキャップが足りない場合、
ファンクションキー等のキーキャップを逆向きにして取り付けて代用するとよいです。

または、無刻印のキーキャップの使用もおすすめです。


キーキャップの取り付けが完了したら、組み立ては完了です。
