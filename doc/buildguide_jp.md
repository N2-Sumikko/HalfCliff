...
(余談)Halfcliffではんだ付けを行う基板はすべて一層(片面)基板なので、本来レジスト(はんだ付けを行う面に塗られる色がついた保護膜)は片面のみなのですが、初期ロットの左右キースイッチ基板は製造時の何らかの理由でレジストが両面にある状態になってしまいました。ビルドガイドでは初期ロットの基板を撮影しているので、ちょっと赤色がきつくてシルクが見づらいです。ごめんなさい。
