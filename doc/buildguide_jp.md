...
JISセット(左右ProMicro基板＋左右キースイッチ基板＋左右JIS拡張基板）の組立方法が記載されています。
...
(文字だけでは分かりにくい部品は、組立の各手順の写真も参考にしてみてください。)
...
# 左右ProMicro基板の組立
左右ProMicro基板を組立ます。(つまり、各作業を2回行うことになります。)
...
画像は主に左ProMicro基板のものですが、右の組立方法も殆ど同じです。
...
以下、はんだ付けする部品は同じように浮きを確認しながら取付けていくとよいです。
# 1x4ロープロファイルピンソケットのはんだ付け
...
2x4細ピンヘッダ(2mmピッチ)を取付け、はんだ付けします。ピン同士がはんだでくっつかないように注意してください。
...
# ジャンパピンの取付け
はんだ付けしたピンヘッダへジャンパピンを取り付けます。画像の位置に取付けてください。
![ジャンパピン取付後](https://user-images.githubusercontent.com/54104281/122669083-96608500-d1f6-11eb-8e9c-5a822e6ed93c.jpg)
...
はんだ付け後、余分なピンをニッパー等で切り落とします。切り落とさないと、後で取付けるProMicroカバーにピンが干渉します。
...
例えば画像の左下のキー等、正方形の穴以外に取付けるキースイッチは、他と比べて浮きやすいので特に注意してください。
...
右キースイッチ基板のLEDテープの貼付けは、基板のバージョンによって内容が変わります。

基板のバージョンは基板にシルクで印刷されています。

バージョンが1.01以降の場合は、左キースイッチ基板と同じ内容(基板とLEDテープのシルクの記述を合わせる)を行います。

バージョンが1.0の場合は、左キースイッチ基板と同じ手順ではLEDテープは動作しません。
…対応として、LEDテープが動作するように改造した基板に、LEDテープをはんだ付け済みの状態で基板を販売します。そのため、右キースイッチ基板のLEDテープはんだ付け手順は不要(になる予定)です。
...
# キースイッチ基板と拡張基板のピンヘッダへのジャンパピンの取付け
キースイッチ基板と拡張基板のピンヘッダが隣り合うようにそれぞれの基板の位置を微調整し、ピンヘッダの各ピンを橋渡しするように、ジャンパピンを取付けます。
...
# キーキャップの取付け
各キースイッチへお好みのキーキャップ(JIS配列のもの)を取付けます。
