...
JISセット(左右ProMicro基板＋左右キースイッチ基板＋左右JIS拡張基板）の組み立て方法が記載されています。(まだ作成中につき注意)
...
(文字だけでは分かりにくい部品は、組み立ての各手順の写真も参考にしてみてください。)
...
M2x2mm中空スペーサー	|	8(予備4)	|	長さ2mmの円柱の形をした部品です。(厚めのワッシャのような部品です。)	|
...
ピンヘッダ1x12	|	4	|	12ピンのピンヘッダです。基本はProMicroの袋に同梱されているものを使います。
...
* ニッパー
* 鉄やすり
* 瞬間接着剤(ピンソケットやピンヘッダの取付け時に使用しました。部品の樹脂部分にのみ少量だけ塗布して基板に接着します。使う場合は金属部分につかないよう注意です。)
...
画像は主に左ProMicro基板のものですが、右の組み立て方法も殆ど同じです。

左右で方法が大きく異なるものには、左用の画像も用意しています。
# (左JIS拡張基板とProMicro基板が繋がっている場合)コントロール基板の切り離し
左JIS拡張基板とProMicro基板が繋がっている場合、ニッパー等で画像の様に(以下、「画像の様に～」は基本的に省略します。)切り離します。
...
ピンソケットが基板から浮いていないことを確認します。画像の状態であればOKです。
![1x9L字ピンソケット浮き確認](https://user-images.githubusercontent.com/54104281/122664713-10d0db00-d1de-11eb-8f03-969f75833915.jpg)
浮いている場合は、はんだを溶かしながら位置を調整します。問題が無ければ残りの端子をはんだ付けします。

以下、はんだ付けする部品は同じように浮きを確認しながら取り付けていくとよいです。
# 1x4ロープロファイルピンソケットのはんだ付け
...
2x4細ピンヘッダ(2mmピッチ)を取り付け、はんだ付けします。ピン同士がはんだでくっつかないように注意してください。

1x12ピンソケットの後のはんだ付けは大変なので、それより前にはんだ付けします。(画像は1x12ピンソケットを先にはんだ付けしてしまっています。やっぱり大変でした。)
...
# ジャンパピンの取り付け
はんだ付けしたピンヘッダへジャンパピンを取り付けます。画像の位置に取り付けてください。
![ジャンパピン取り付け後](https://user-images.githubusercontent.com/54104281/122665391-dbc68780-d1e1-11eb-91df-f8383d642f81.jpg)

(余談)ジャンパピンの位置を変更することで、左右のキーボード間の通信をI2Cで行うこともできます。

ただし、通信をI2Cで行う場合、ファームウェアの変更が必要となります。方法を知っている方向けです。(需要があれば詳細を追記するかもです。)
# TRRSジャックのはんだ付け
基板へTRRSジャックを取り付け、はんだ付けします。左右の基板で取付場所が異なるので注意してください。

…といっても、順番通り組み立てれば、使用しない取付場所は1x9L字ピンヘッダで塞がれるので問題無いはずです。
...
また、HalfCliffではコンスルーは使用不可ですので注意してください。
...
はんだ付け後、余分なピンをニッパー等で切り落とします。切り落とさないと、後で取り付けるProMicroカバーにピンが干渉します。
...
# ダイオードのはんだ付け
リードタイプのダイオードを左右のキースイッチ基板に取付け、はんだ付けします。
...
(余談)Halfcliffに使用する基板はすべて一層(片面)基板なので、本来レジストは片面のみなのですが、初期ロットの左右キースイッチ基板のみ製造時の何らかの理由でレジストが両面にある状態になってしまいました。ビルドガイドでは初期ロットの基板を撮影しているので、ちょっと赤色がきつくてシルクが見づらいです。ごめんなさい。
# 1x9L字ピンヘッダ2つのはんだ付け
基板に1x9L字ピンヘッダ2つを取り付け、はんだ付けします。L字ピンヘッダの取付け向きに注意してください。

言葉で説明すると、上から見てL字ピンヘッダの樹脂部分の細長い面が見えるよう、かつピンヘッダの端子が基板の外に向くように取付けます。
...
# 1x4ロープロファイルピンソケットのはんだ付け
左右キースイッチ基板に1x4ロープロファイルピンソケットを取り付け、はんだ付けします。
...
![右キースイッチ基板_1x8LPピンヘッダはんだ付け後](https://user-images.githubusercontent.com/54104281/122666514-b38e5700-d1e8-11eb-9abb-847ff799e83d.jpg)
![左キースイッチ基板_1x9LPピンソケットはんだ付け後](https://user-images.githubusercontent.com/54104281/122666542-cf91f880-d1e8-11eb-8c7d-c53577474a3f.jpg)
# ProMicro基板とキースイッチ基板の接続
...
左右のトッププレートへM2x7mmスペーサーを、M2x6mmねじで取り付けます。
...
正しく取付けたら、いずれか1つのキースイッチの2つのピンのうち片方を、キースイッチ基板から浮かないようにはんだ付けします。

例えば画像の左下のキー等、正方形の穴以外に取り付けるキースイッチは、他と比べて浮きやすいので特に注意してください。
...
(補足)ただしVer1.0の右キースイッチ基板のみ、基板のパターンミスでこの向きにLEDテープをはんだ付けしても動作しません。
ですので、現時点では初期ロットの右キースイッチ基板はLEDテープ非対応、とご案内せざるを得ません。
対処方法を考えており、決まり次第ビルドガイドに追記します。少々お待ちください。

LEDテープの向きを合わせたら、左右キースイッチ基板へLEDテープを貼り付けます。
...
取付位置は画像を参考にしてください。(画像は右キースイッチ基板のものですが、左キースイッチ基板も位置は同じです。)
...
# (左のみ）ProMicro基板と繋がっていた部分の整形、やすりがけ
左基板のProMicro基板と繋がっていた部分を、ProMicro基板の時と同じように、鋭い部分が残らないように切り離します。
...
右基板に1x9ロープロファイルピンヘッダを取付け、はんだ付けします。
...
JIS拡張基板へM2x7mmスペーサー4つ(左右で合計8個)を取り付けます。
...
* ボトムプレートへの拡張基板の仮止め
* キースイッチ基板と拡張基板のピンヘッダへのジャンパピンの取付け
* ボトムプレートと各基板のねじ止め
...
キースイッチ基板と拡張基板のピンヘッダが隣り合うようにそれぞれの基板の位置を微調整し、ピンヘッダの各ピンを橋渡しするように、ジャンパピンを取り付けます。
...
# キースイッチ基板のトッププレートへのProMicroカバーのねじ止め
キースイッチ基板へ取り付けてあるM2x7mmスペーサーに、ProMicroカバーをM2x4mmねじ2本でねじ止めします。
![左基板_ProMicroカバーねじ止め後](https://user-images.githubusercontent.com/54104281/122668549-93b06080-d1f3-11eb-9298-bfc938806837.jpg)

(補足)もしM2x7mmスペーサーの取り付けを忘れている場合は、M2x3㎜スリムヘッド小ねじを入手し、基板とトッププレートの間からそのねじを入れ、M2x7mmスペーサー2つをねじ止めした後、M2x6㎜ねじでProMicroカバーをねじ止めします。
# ボトムプレートへのゴム足4つの取り付け
...
一番下等に使用する1uのキーキャップが足りない場合、ファンクションキー等のキーキャップを逆向きにして代用するのもよいです。

他には、無刻印のキーキャップの使用もおすすめです。
![キーキャップ取付後](https://user-images.githubusercontent.com/54104281/122668616-e1c56400-d1f3-11eb-9bda-4cc04adff9ad.jpg)
# 組立後の動作確認
