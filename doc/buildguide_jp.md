...
基本セット＋拡張基板(JIS）セットの組み立て方法が記載されています。
(まだ作成中につき注意)
...
|名前|合計数|備考|
|:-|:-|:-|
ProMicro基板|	2	|	表にControl Boardと書いてある基板です。左拡張基板(JIS)と繋がっています。|
...
|名前|合計数|備考|
|:-|:-|:-|
基板(左キーボード）	|	1	|	表にLeft Switch Boardと書いてある基板です。
基板(右キーボード）	|	1	|	表にRight Switch Boardと書いてある基板です。
...
|名前|合計数|備考|
|:-|:-|:-|
左拡張基板(JIS)	|	1	|	表にLeft Extension Boardと書いてある基板です。ProMicro基板と繋がっています。
右拡張基板(JIS)	|	1	|	表にRight Extension Boardと書いてある基板です。
...
ピンの位置を変更することで、左右のキーボード間の通信をI2Cで行うこともできます。
...
リードを曲げたら、基板へダイオードを取り付けます。取り付け(とはんだ付け)は5個ずつくらいで行うとよいです。
...
# 1x4ピンソケット(ロープロファイル)のはんだ付け
左の基板に1x4ピンソケット(ロープロファイル)を取り付け、はんだ付けします。
...
動作確認完了後、組み立て進行のためにProMicro基板とキースイッチ基板を分離します。
...
ダイオードは右基板で10個、左基板で15、合計25個使用します。

取り付けはキースイッチ基板の時と同様に行います。

ただし、ダイオードの向きが
キースイッチ基板とは逆なので注意してください。
...
# 各基板の連結、ボトムプレート、ProMicroカバーの取り付け
各基板を連結し、ボトムプレートを取り付けます。(ry
...
# キースイッチ基板と拡張基板のピンヘッダへのジャンパピンの取り付け
キースイッチ基板と拡張基板のピンヘッダが隣り合うように各基板の位置調整し、
...
ProMicro基板を固定するねじについては、M2のナットを固定しながらでないと本締め出来ないので注意してください。

# キースイッチ基板のトッププレートへのProMicroカバーのねじ止め
キースイッチ基板へ取り付けてあるM2x7mmスペーサーに、ProMicroカバーをM2x3mmねじでねじ止めします。
