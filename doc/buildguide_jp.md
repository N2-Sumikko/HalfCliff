...
(余談)Halfcliffではんだ付けを行う基板はすべて一層(片面)基板なので、本来レジストは片面のみなのですが、初期ロットの左右キースイッチ基板のみ製造時の何らかの理由でレジストが両面にある状態になってしまいました。ビルドガイドでは初期ロットの基板を撮影しているので、ちょっと赤色がきつくてシルクが見づらいです。ごめんなさい。
# 1x9L字ピンヘッダ2つのはんだ付け
