...
右キースイッチ基板のLEDテープの貼付けは、基板のバージョンによって内容が変わります。基板のバージョンは基板の表面にシルクで「Ver～～」と印刷されています。

バージョンが1.01以降の場合は、左キースイッチ基板と同じ手順(基板のシルクとLEDテープの文字を対応させる)を行います。

バージョンが1.0の場合は、基板のパターンミスのせいで、左キースイッチ基板と同じ手順ではLEDテープは動作しません。
そのため、LEDテープが動作するように改造した右キースイッチ基板に、LEDテープをはんだ付け済みのものを販売しています。そのため作業は不要です。
