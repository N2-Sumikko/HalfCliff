...
あとで触ってケガしないよう、鋭い部分が残らないように切り離してください。
...
左右の基板で取付位置が異なるので注意してください。
…といっても、順番通りに組み立てれば、使用しない取り付け場所はL字ピンヘッダで塞がれるので問題ないかと思います。)
...
リードを曲げたら、基板へダイオードを取り付けます。

取り付けは5個ずつくらいで行うとよいです。

取り付けの際は、基板のシルクの向きにダイオードの黒色の部分を合わせます。

また、右手用のダイオード2つのみ、ほかのダイオードとは離れた場所にあるので注意してください。
