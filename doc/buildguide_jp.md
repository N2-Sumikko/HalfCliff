...
# コントロール基板の切り離し
(左JIS拡張基板とProMicro基板が繋がっている場合)ProMicro基板と左拡張基板(JIS)をニッパー等で画像の様に(以下、「画像の様に～」基本的に省略します。)切り離します。
...
ただし、通信をI2Cで行う場合、ファームウェアの変更が必要となります。方法を知っている方向けです。(詳細は追記するかもです。)
...
(キースイッチはんだ付け後に行う時は、問題発生時の原因がキースイッチであることも考慮しておきます。）
...
入っていない場合は、ピンが曲がっていないか等を確認します。

正しく取り付けたら、任意のキースイッチの2つのピンの片方を、

キースイッチ基板から浮かないようにしてはんだ付けします。

例えば画像の左下のキー等、正方形の穴以外に取り付けるキースイッチは、他と比べて浮きやすいので特に注意してください。

マスキングテープで基板にキースイッチを固定したり、大き目のクリップで基板とキースイッチを挟む等の方法がおすすめです。

端子とパッド間は十分な量のはんだではんだ付けしてください。(画像のはんだの量なら大丈夫です。)
