こちらはHalfcliffのオプションの部品用の資料です。(2021/12/31時点では作成中の資料です。資料作成完了、と書かれたもの以外は使用不可です。)

対応を予定しているオプションの部品は以下の通りです。
設計時の制約によっては、対応を断念する可能性もあります。
* (現状は左キーボードのみ対応。資料作成完了)ロータリーエンコーダ
...
取付後、キースイッチ基板と拡張基板を接続しているジャンパピンの取付状態を確認します。外れたり抜けかかっている場合は、再度取付けます。
## (QMK Toolboxを使用する場合)ロータリーエンコーダ対応のファームウェアの書込み
キーボード組立時に行った
...
トラックボールモジュール取付用キットは用意出来次第販売する予定です。
...
言葉で説明すると、トラックボールモジュールの配線を自分の方に向け、変換基板の部品がある面を下、かつ部品が無い面の文字を自分から読める向きにした状態で、部品が無い面からある面へ配線を通します。

その後、部品がある面の長方形のパッドにそれぞれの配線をはんだ付けします。

ピンヘッダ1x4については、ピンヘッダの短い方が変換基板側、、かつピンヘッダの樹脂部分が変換基板の部品がある面に来るようにして、変換基板のNC端子以外にピンヘッダを取付けてはんだ付けします。

(四角の端子にピンヘッダを差し込めば、NC端子にピンヘッダが取付けられていない状態になります。)
