...
* ロータリーエンコーダ操作時の動作を変更する場合、QMK MSYS、またはRemap(もしくはVIA)を使用する必要があります。Remap(もしくはVIA)を使用する場合、ロータリーエンコーダを取付けた列の一番上のキーは、ロータリーエンコーダを時計回り(反時計回りかも)に回転させたキーと同じになるので注意してください。
* (β版のみ)ロータリーエンコーダを取付け時にエンコーダの端子とトッププレートが干渉する場合、端子の位置や角度をと干渉しないよう調整するか、トッププレートの干渉部分を削る必要があります。
...
にキースイッチが取付済の場合は取外します。取付けていない場合はこの手順は飛ばします。

まずは取付を行うキーボードを裏返し、キースイッチ基板、拡張基板をボトムプレートに固定しているねじを取外します。

左キーボードのねじ取外しのイメージ(黄丸で示したねじを取外します。右の場合も同じイメージです。)
...
左キーボードでの取外し箇所(黄枠の箇所のキースイッチを取外します。右の場合は逆のイメージです。)

![ロータリーエンコーダ用キースイッチ取外し箇所](https://user-images.githubusercontent.com/54104281/147804005-3077c985-11f4-4bf2-b03b-e66974970ed4.jpg)
...
|フォルダ|ファイル名|概要|
|:-|:-|:-|
|hex/qmk_0.15.4/via/others/(unofficial)oled_encoder_trackball_mousekey/|halfcliff_via.hex|OLED、ロータリーエンコーダ、トラックボール使用時用。エンコーダの機能はVIA、Remapにて変更可能|
|hex/qmk_0.15.4/via/others/(unofficial)encoder/|halfcliff_via.hex|ロータリーエンコーダ使用時用。エンコーダの機能はVIA、Remapにて変更可能|
## (QMK MSYSを使用する場合)ロータリーエンコーダ対応のファームウェアの書込み
編集中です。
