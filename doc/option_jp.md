...
言葉で説明すると、トラックボールモジュールの配線を自分の方に向け、変換基板の部品がある面を下、かつ部品が無い面の文字を自分から読める向きにした状態で、部品が無い面からある面へ配線を通します。その後、部品がある面の長方形のパッドにそれぞれの配線をはんだ付けします。

ピンヘッダ1x4については、ピンヘッダの短い方が変換基板側、、かつピンヘッダの樹脂部分が変換基板の部品がある面に来るようにして、変換基板のNC端子以外にピンヘッダを取付けてはんだ付けします。(四角の端子にピンヘッダを差し込めば、NC端子にピンヘッダが取付けられていない状態になります。)
