...
2022/1/4時点で使用可能なロータリーエンコーダ用のHexファイルは以下の通りです。

QMKにマージされたファームウェアを変更して作成したものには、(unofficial)と記載しています。いずれも動作確認は完了済みです。(動作に問題があった場合はご連絡ください。)
...
* キースイッチ基板のロープロファイルピンソケット1x4へ変換基板のピンヘッダ1x4を取付
...
ピンヘッダ1x4については、ピンヘッダの短い方が変換基板側、、かつピンヘッダの樹脂部分が変換基板の部品がある面に来るようにして、四角形のランドにピンが入るようにピンヘッダ1x4を取付けてはんだ付けします。(四角形のランドから一番遠いランドはNC端子なので、ピンヘッダの取付け、はんだ付けは不要です。N)
