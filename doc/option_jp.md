...
## 使用時の主な注意点
* 組立済みのキーボードにロータリーエンコーダ(長いので以下RE)を取付ける場合、取付け箇所のキースイッチを(はんだを除去して)取外す必要があります。
* REのプッシュ機能には対応していません。
* REを使用する場合、対応するファームウェアの書込みが必要です。
* 操作時の動作を変更する場合、QMK MSYSまたはRemap、VIAを使用する必要があります。Remap、VIAを使用する場合、REを取付けた列の一番上のキーは、REを時計回り(反時計回りかも)に回転させたキーと同じになるので注意してください。
* (β版のみ)RE取付け時にREの端子とトッププレートが干渉する場合、端子の位置や角度をと干渉しないよう調整するか、トッププレートの干渉部分を削る必要があります。
...
|フォルダ|ファイル名|概要|
|:-|:-|:-|
|hex/qmk_0.15.4/via/others/(unofficial)oled_encoder_trackball_mousekey/|halfcliff_via.hex|OLED、RE、トラックボール併用時。エンコーダの機能はVIA、Remapにて変更可能|
