...
* (対応完了。随時追記)ロータリーエンコーダ
* (現状は左キーボードのみ対応完了。部品は準備中)ビットトレードワン製トラックボール(ADTB7M)
* (対応完了。随時追記)OLED
* (動作確認予定)BLE Micro Pro
* (試作品作成完了)直交配列拡張基板
* (設計予定)英字配列風拡張基板
* (必要に応じて資料作成予定)右キーボードのテンキーとしての組立


# ロータリーエンコーダ
基板のフットプリントに対応しているロータリーエンコーダが使用可能です。(プッシュ機能には未対応です。)
...
## 必要な部品の用意
以下の部品を用意します。(トラックボールモジュール取付用キットが用意出来ていないので、現時点では参考程度の情報となります。β版を購入された方で、取付をご希望されている方がおりましたら、お手数ですがご連絡いただければと思います。)
|名前|合計数|備考|
|:-|:-|:-|
...
* halfcliffにてトラックボールを使用する場合、通常のファームウェア(本資料に記載のHexファイル)ではトラックボールを左キーボードに取付けた時のみ正しく動作します。右キーボードにトラックボールを取付けたい場合は、右キーボードへのUSBケーブル接続時に正常動作するファームウェアの書込みが必要です。(具体的には、config.hに#define MASTER_RIGHTを追記してコンパイルしたファームウェア等です。)
...
その後、画像を参考にしながら部品がある面の長方形のパッドにそれぞれの配線をはんだ付けします。

ピンヘッダ1x4については、ピンヘッダの短い方が変換基板側、かつピンヘッダの樹脂部分が変換基板の部品がある面に来るようにして、四角形のランドにピンが入るように取付けてはんだ付けします。(四角形のランドから一番遠い丸いランドにピンが入らないようにします。)
...
その後画像の様に、トラックボールモジュール用ボトムプレートの四角い穴へ、トラックボールモジュールの配線を通します。
...
画像の様に、トラックボールモジュール用ボトムプレートの穴にM2x10mmねじ2本を通すようにして取付けます。
...
(注意点)ProMicro基板のロープロファイルピンソケット1x4へOLEDのピンヘッダ1x4を取付ける際には、ある程度の高さが必要です。(具体的には、OLEDの基板底面から樹脂部分含めて9㎜弱の長さが必要です。[こちら](https://hirosugi.co.jp/products/N/PSH-41.html)等が使用可能です。
...
ProMicro基板を固定していたM2x7mmスペーサー2つの上にM2x2mm中空スペーサーをそれぞれ置きます。左右のキーボードに対して同様の作業を実施します。

(画像はトラックボール、ロータリーエンコーダを取付けした状態の左キーボードです。)
