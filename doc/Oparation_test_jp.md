...
本資料の内容はWindows(厳密にはWindows10)にてQMK ToolboxまたはQMK MSYSを使用する場合を基準にしています。
...
クリック後、表示されたダイアログにて、画像の赤枠で囲んだ部分(フォルダのパス。フォルダの場所を示すもの）をコピーしてメモ帳等に貼り付けます。
...
qmk setupの実行中、何度か[y/n]の確認メッセージが出ますが、これらに対してはメッセージが出る度にyを入力後Enterを押します。(qmk setupの完了にはしばらく時間がかかります。)
...
(viaをdefaultにするとdefaultのキーマップが書込み出来ますが、その場合VIAやRemap等の便利なサービスを利用出来ません。特別な理由がない限りviaの書込みを推奨します。）
...
(余談)もし右キーボードのProMicroにUSBケーブルを接続して使用したい場合は、QMKのドキュメント
...
(ピンセットの代わりに、押した状態のキースイッチを使うのも良いです。スイッチがはんだ付け済みの場合はスイッチを押します。)
